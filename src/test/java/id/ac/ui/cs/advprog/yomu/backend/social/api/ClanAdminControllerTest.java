...
class ClanAdminControllerTest {
  ...
  void listClans_returnsOkWithClanList() throws Exception {
    ...
    mockMvc
        .perform(get("/api/admin/social/clans"))
  ...
  void listMembers_returnsUsernameResolvedFromPort() throws Exception {
    ClanMember leader =
        new ClanMember(UUID.randomUUID(), clanId, leaderId, ClanMemberRole.LEADER, Instant.now());
    ClanMember member =
        new ClanMember(UUID.randomUUID(), clanId, memberId, ClanMemberRole.MEMBER, Instant.now());
    ...
    mockMvc
        .perform(get("/api/admin/social/clans/{clanId}/members", clanId))
        .andExpect(status().isOk())
  ...
  void deleteClan_returnsNoContent() throws Exception {
    ...
    mockMvc
        .perform(delete("/api/admin/social/clans/{clanId}", clanId))
  ...
  void removeMember_returnsNoContent() throws Exception {
    ...
    mockMvc
        .perform(delete("/api/admin/social/clans/{clanId}/members/{userId}", clanId, memberId))
        .andExpect(status().isNoContent());
  ...
  void listJoinRequests_returnsOk() throws Exception {
    ...
    mockMvc
        .perform(get("/api/admin/social/join-requests"))
        .andExpect(status().isOk())
