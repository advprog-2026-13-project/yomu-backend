...
class AdminAchievementControllerTest {
    ...
    @Mock
    private AchievementAdminService adminService;

    @InjectMocks
    private AdminAchievementController controller;
    ...
    void testCreateAchievement() throws Exception {
        CreateAchievementRequest request = new CreateAchievementRequest("Test", "Desc", AchievementType.READING_COMPLETED, 10);
        Achievement achievement = new Achievement(UUID.randomUUID(), "Test", "Desc", AchievementType.READING_COMPLETED, 10);
        
        when(adminService.createAchievement(anyString(), anyString(), any(AchievementType.class), anyInt())).thenReturn(achievement);
    ...
    void testDeleteAchievement() throws Exception {
        ...
        mockMvc.perform(delete("/api/admin/achievements/" + id))
                .andExpect(status().isNoContent());
    }

    @Test
    void testCreateDailyMission() throws Exception {
        CreateDailyMissionRequest request = new CreateDailyMissionRequest("Mission", "Desc", AchievementType.QUIZ_COMPLETED, 5);
        DailyMission mission = new DailyMission(UUID.randomUUID(), "Mission", "Desc", AchievementType.QUIZ_COMPLETED, 5);

        when(adminService.createDailyMission(anyString(), anyString(), any(AchievementType.class), anyInt())).thenReturn(mission);

        mockMvc.perform(post("/api/admin/achievements/daily-missions")
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(request)))
                .andExpect(status().isCreated())
    ...
    void testDeleteDailyMission() throws Exception {
        UUID id = UUID.randomUUID();
        mockMvc.perform(delete("/api/admin/achievements/daily-missions/" + id))
                .andExpect(status().isNoContent());
    }
}
