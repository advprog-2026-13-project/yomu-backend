...
public class UserAchievementController {
    ...
    public ResponseEntity<List<UserAchievementProgress>> getUserAchievementProgress(@PathVariable UUID userId) {
        return ResponseEntity.ok(queryService.getUserAchievementProgress(userId));
    }

    @GetMapping("/users/{userId}/daily-progress")
    public ResponseEntity<List<UserDailyMissionProgress>> getUserDailyMissionProgress(@PathVariable UUID userId) {
        return ResponseEntity.ok(queryService.getUserDailyMissionProgressForToday(userId));
    }

    @GetMapping("/users/{userId}/completed")
    public ResponseEntity<List<UserAchievementProgress>> getCompletedAchievements(@PathVariable UUID userId) {
