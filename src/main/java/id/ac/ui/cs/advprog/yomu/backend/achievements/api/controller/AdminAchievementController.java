...
public class AdminAchievementController {
    ...
    @PostMapping
    // Usually you'd uncomment this when security is fully integrated: @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<Achievement> createAchievement(@Valid @RequestBody CreateAchievementRequest request) {
        Achievement created = adminService.createAchievement(
                request.getName(),
    ...
    public ResponseEntity<DailyMission> createDailyMission(@Valid @RequestBody CreateDailyMissionRequest request) {
        DailyMission created = adminService.createDailyMission(
                request.getName(),
                request.getDescription(),
